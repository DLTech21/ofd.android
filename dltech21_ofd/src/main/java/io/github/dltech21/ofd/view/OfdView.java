package io.github.dltech21.ofd.view;
...
public class OfdView extends RelativeLayout {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (changeListener != null) {
                changeListener.onTouchUp(OfdView.this, qualityTag);
            }
        }
    };
    ...
    public OfdView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public OfdView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public OfdView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }

    public OfdView(Context context, int pageNum, OfdManager ofdManager) {
        super(context);
...
//                if (!ofdManager.isCheckSign()) {
                    if (mSize != null) {
                        ...
                        if (mWidgetAreas != null) {
                            ...
                            if (hit) {
                                String id = mWidgetAreas.getSignId()[currentWidget];
                                SignatureInformation info = ofdManager.getSignatureInformationBySid(pageNum, id);
                                if (info != null) {
...
//                    }
        ...
        contentView.setOnViewDownListener(new OnViewListener() {
            ...
            public void onViewDown(View view, float x, float y) {
                if (mSize != null && mWidgetAreas != null) {
                    RectF rectF = new RectF();
                    boolean hit = false;
                    float scale = contentView.getDisplayRect().width() / mSize.x;
                    final float docRelX = (x - contentView.getDisplayRect().left) / scale;
    ...
    private void getSigntureAreas() {
        AsyncTask<Void, Void, WidgetArea> mLoadWidgetAreas = new AsyncTask<Void, Void, WidgetArea>() {
            ...
            protected WidgetArea doInBackground(Void... arg0) {
                return ofdManager.getWidgetAreas(pageNum);
            }
